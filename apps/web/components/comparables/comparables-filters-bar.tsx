...
interface ComparablesFiltersBarProps {
  search: string | undefined
  comparableType: string | undefined
  state: string | undefined
}

export function ComparablesFiltersBar({
  search,
  comparableType,
  state,
}: ComparablesFiltersBarProps) {
  const [mobileOpen, setMobileOpen] = useState(false)

  const activeFilterCount = useMemo(() => {
    return [search, comparableType, state].filter(Boolean).length
  }, [comparableType, search, state])

  const hasFilters = Boolean(search || comparableType || state)
  ...
  const formBody = (
    <>
      <input
        ...
        name="search"
        defaultValue={search}
        placeholder="Search address, city, state, source, or notes…"
  ...
  return (
    <>
      <div className="hidden xl:block">
        <form
          className="surface-card grid gap-3 rounded-[28px] p-4 xl:grid-cols-[minmax(0,1.6fr)_minmax(0,0.85fr)_minmax(0,0.85fr)_auto_auto]"
          method="GET"
        >
          {formBody}
        </form>
      ...
      {mobileOpen ? (
        <ModalShell
          title="Filter Comparables"
          description="Narrow the market evidence list without taking over the page."
